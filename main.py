...
d = (
x  8 - 16  x  7 + 196  x  6 - 1456  x  5 + 9796  x 
4 - 44320  x  3 + 87040  x  2 - 44800  x + 6400)
a = 40  (x  2 - 4  x + 8)
return a, b, d
def koeff(x1, p3)
p1 = 2
...
j1[i], jacobi1[i] = jacobi(x1[i], x2_1[i], x3_1[i], x4_1[i],
p31[i])
j1_sol[i] = max(j1[i])  0
if j1_sol[i] != j1_sol[i - 1] and i != 0 and j1_sol[i - 1] != -1
and j1_sol[i] != -1
b1.append(x1[i])
p32[i] = (-b - math.sqrt(d))  a
x2_2[i], x3_2[i], x4_2[i] = koeff(x1[i], p32[i])
j2[i], jacobi2[i] = jacobi(x1[i], x2_2[i], x3_2[i], x4_2[i],
p32[i])
j2_sol[i] = max(j2[i])  0
if j2_sol[i] != j2_sol[i - 1] and i != 0 and j2_sol[i - 1] != -1
and j2_sol[i] != -1
b2.append(x1[i])
else
p31[i] = 0
p32[i] = 0
return x1, p31, p32, j1, j2, x2_1, x2_2, x3_1, x3_2, x4_1, x4_2, j1_sol,
j2_sol, b1, b2, n, jacobi1, jacobi2
...
dx4_2 = np.zeros(n)
for i in range(n)
dx1_1[i], dx2_1[i], dx3_1[i], dx4_1[i] = examfun(x1[i], x2_1[i],
x3_1[i], x4_1[i], p31[i])
dx1_2[i], dx2_2[i], dx3_2[i], dx4_2[i] = examfun(x1[i], x2_2[i],
x3_2[i], x4_2[i], p32[i])
return dx1_1, dx1_2, dx2_1, dx2_2, dx3_1, dx3_2, dx4_1, dx4_2
print(---------------------MAIN---------------------------)
x1, p31, p32, j1, j2, x2_1, x2_2, x3_1, x3_2, x4_1, x4_2, j1_sol, j2_sol, b1,
b2, n, jacobi1, jacobi2 = solvex()
print(b1, b2)
